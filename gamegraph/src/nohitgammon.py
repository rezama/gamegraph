...
class NohitGammonState(object):
    ...
    def __init__(self, exp_params, player_to_move):
        ...
        if self.is_graph_based:
            if NohitGammonState.GAME_GRAPH is None:
                filename = exp_params.get_graph_filename(Domain.name)
                NohitGammonState.GAME_GRAPH = StateGraph.load_from_file(filename)
...
#                   (checker2_pos == self.BOARD_OFF)
    ...
    def generate_graph(cls, exp_params):
        ...
        while not Q.empty():
            ...
            for roll in NohitGammonDie.SIDES:
                s.roll = roll
                must_consider_forfeit = True
                for action in NohitGammonAction.ALL_ACTIONS:
                    if (action != NohitGammonAction.ACTION_FORFEIT_MOVE) or must_consider_forfeit:
...
class NohitGammonAgentRandom(NohitGammonAgent):
    
    def __init__(self):
        super(NohitGammonAgentRandom, self).__init__()
    
    def select_action(self):
        return NohitGammonAction.random_action(self.state)
    
class NohitGammonAgentNeural(NohitGammonAgent):
    ...
    def select_action(self):
        ...
        if len(action_values) > 0:
            ...
            action = NohitGammonAction.ACTION_FORFEIT_MOVE
