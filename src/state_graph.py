...
class StateGraph(object):
    ...
    def save(self, exp_params, filename_suffix=None):
        if exp_params.is_first_trial():
            ...
            print 'Saving graph for signature: %s, suffix: %s...' % (
                exp_params.signature, filename_suffix)
            graph_filename = exp_params.get_graph_filename()
    ...
    def get_random_successor_at_distance(self, node_id, color, distance):
        ...
        while tries_left > 0:
            ...
            if successors:  # len(successors) > 0:
        ...
        while cont:
            ...
            for node_id in reversed(range(self.get_num_nodes())):
                ...
                if (node_id not in self.sinks[PLAYER_WHITE] and
                            ...
                            if successor is not None:
                                ...
                                if roll_values[roll_index] is None:
                                    ...
                                    if node_color == PLAYER_WHITE:
                                        ...
                                        if successor_value < roll_values[roll_index]:
                                            roll_values[roll_index] = successor_value
                            # if (multiplier * successor_value >
                            #         multiplier * roll_values[roll_index]):
                            #     roll_values[roll_index] = successor_value
                    ...
                    #         self.node_names[node_id], self.node_attrs[node_id][VAL_ATTR],
                    #         new_state_value)
            ...
            print 'maximum residual: %.5f' % max_residual
    ...
    def compute_dice_volatility(self, exp_params):
        ...
        for node_id in range(self.get_num_nodes()):
            ...
            if (node_id not in self.sinks[PLAYER_WHITE] and
                ...
                squared_diff_choose_roll_adjusted = (
                    (value_with_choose_roll - mean_value) *
                    (value_with_choose_roll - mean_value) *
                    exp_params.choose_roll)
                squared_diff_sum = (
                    sum(squared_diffs_regular_adjusted) +
                    squared_diff_choose_roll_adjusted)
    ...
    def compute_action_volatility(self, exp_params):  # pylint: disable=unused-argument
        sum_volatility = 0.0
        count_nodes = 0
        for node_id in range(self.get_num_nodes()):
            if (node_id not in self.sinks[PLAYER_WHITE] and
                ...
                for roll in self.all_rolls:
                    ...
                    if roll_successors:  # len(roll_successors) > 0:
                        count_nodes += 1
                ...
                for action in self.all_actions:
                    ...
                    if successor is not None:
                        ...
                        if VAL_ATTR in self.node_attrs[node_id]:
                            flags += '(value: %.2f -> %.2f)' % (
                                self.node_attrs[node_id][VAL_ATTR],
                                self.node_attrs[successor][VAL_ATTR])
                        if '0' in self.node_names[successor][2:]:
                            flags += '-HIT'
                        if successor_dist < node_dist:
                            flags += '-BACK'
                            new_successor = self.get_another_successor_check_distance(
                                node_id, successor)
                ...
                for action in self.all_actions:
                    ...
                    if successor is not None:
                        ...
                        if successor_dist < node_dist:
                            ...
                            #     print 'Replacing edge %s -> %s' % (
                            #         self.node_names[node_id],
                            #         self.node_names[new_successor])
                            # else:
    ...
    def remove_attrs(self):
        for node_id in range(self.get_num_nodes()):
            if BFS_COLOR_ATTR in self.node_attrs[node_id]:
                del self.node_attrs[node_id][BFS_COLOR_ATTR]
            if DIST_ATTR in self.node_attrs[node_id]:
    ...
    def print_stats(self):
        ...
        print 'Number of sources: %d' % (len(self.sources[PLAYER_WHITE]) +
                                         len(self.sources[PLAYER_BLACK]))
        print 'Number of sinks: %d' % (len(self.sinks[PLAYER_WHITE]) +
                                       len(self.sinks[PLAYER_BLACK]))
        total_edges = 0
        for node_id in range(self.get_num_nodes()):
            for roll in self.all_rolls:
                roll_index = roll - self.roll_offset
